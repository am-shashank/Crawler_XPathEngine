...
/**
 * Parses the robots.txt and stores the information specific to cis455Crawler
 * @author Shashank
 *
 */
			...
			if(!responseLine.startsWith("#")) {
				...
				if(temp.length == 2) {
					...
					if(temp[0].equalsIgnoreCase("user-agent")) {
						if(isCis455Crawler == false) {
							if(temp[1].equalsIgnoreCase("cis455Crawler")) {
								clear();
								isCis455Crawler = true;
							} else if(temp[1].equals("*")) {
								isValidUserAgent = true;
								isCis455Crawler = false;
							}
							userAgent = temp[1];	
						}else {
							break;
						}
					} else if(isValidUserAgent == true || isCis455Crawler == true) {

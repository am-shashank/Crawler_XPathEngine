	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(session != null) {
			...
			try {
				...
				} else if(deleteChannel != null) { // check if request is to delete the channel
					try {
						if(deleteChannel != "") {
		...
		message += "<a href=\"HW2/home\">    Click Here!</a></h3>"
				+ "</body></html>";
		pw.write(message);
	}
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			...
			String html = "<!DOCTYPE html> " + "<html> " + "<body> "
						...
						+ "<hw3> <b>Create Channel</b></h3><br/>"
						+ "<form action=\"\" method=\"POST\"> "
						+ "Channel Name:<br>"
						+ "<input type=\"text\" name=\"channelname\"> " + "<br>"
						+ "XSL URL:<br> "
						+ "<input type=\"text\" name=\"xslurl\"><br> "
						+ "XPaths: ; seperated <br> "
			...
			for(String channelName: userChannels) {
				html += "<a href=\"/HW2/displaychannel?channelname="+ channelName + "\">"
						+ channelName+"</a>"
						+ "<form action = \"\" method=\"POST\">"
						+ "<button type=\"submit\" name=\"deletechannel\" value=\""+channelName+"\">Delete "+ channelName+" </button>"
						+ "</form><body>";
			}
			html += "</ul>";
